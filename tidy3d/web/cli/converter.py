...
def _to_um(array: list) -> list:
    ...
    for number_str in array:
        if number_str == "None":
            ...
            if len(number_power) != 2:  # number_str is a variable name with an 'e'
                ...
                if _is_int(number):
                    if _is_int(power):
                        ...
                        value_str = _clean("{:e}".format(value))
            ...
            value_str = _clean("{:e}".format(int(number_str)))
            output_array.append(value_str)
        elif _is_float(number_str):
            value_str = _clean("{:e}".format(float(number_str)))
            output_array.append(value_str)
...
def _addrect(Lines, i: int, rect: int) -> str:
    ...
    while Lines[i][0] == "\n" or Lines[i][:5] == "set('" or Lines[i][0] == "#":
        if Lines[i][5:9] == "name":
            ...
            medium = Lines[i][15:].split(";")[0][:-1] + " ## check material library"
        i += 1
        if i == len(Lines):
            break
...
def _addpoly(Lines, i: int, polyslab: int):
    ...
    poly_string += "\t\tvertices=" + vertices + "*1e-6,\n"
...
def _get_BCs(dim_bc_file: str):
    dim_bc, dim_periodic, dim_sym = "td.PML(num_layers=", False, "0"
    ...
    return dim_bc, dim_periodic, dim_sym
...
def _addfdtd(Lines, i: int, v_dict: dict) -> str:
    ...
    x_periodic, y_periodic, z_periodic = False, False, False
    while Lines[i][0] == "\n" or Lines[i][:5] == "set('" or Lines[i][0] == "#":
        if Lines[i][5:7] == "x'":
            x = Lines[i][8:].split(";")[0][:-1]
        elif Lines[i][5:7] == "y'":
...
def _addmodeexpansion(Lines, i: int, modemon: int):
    ...
    while Lines[i][0] == "\n" or Lines[i][:5] == "set('" or Lines[i][0] == "#":
        ...
        if i == len(Lines):
            break
    mode_monitor_string += num_modes + ") # check if solving for correct mode!\n"
...
def lsf_reader(filename=str) -> None:
    ...
    if filename[-4:] != ".lsf":
        print("Needs .lsf format")
    try:
        file_lsf = open(filename)
    ...
    for i in range(0, len(Lines)):
        Lines[i] = _remove_spaces(Lines[i])
    ...
    for i in range(0, len(Lines)):
        ...
        if line[0] == "#":
            ...
            tidy3d_file += "# " + line[:-1] + " # does not yet parse to Tidy3d\n"
...
# new_file_name = "new_tidy3d_file.py"
# file_to_read = "example.lsf"


def converter_arg(lsf_file, new_file):
    if ".lsf" not in lsf_file:
        print("Error:", lsf_file, "must be an .lsf file.")
        return
    elif ".py" not in new_file:
        print("Error:", new_file, "must be a .py file")
    else:
        file_string = lsf_reader(lsf_file)
        if file_string == "File not found!":
            return
